...
class DB_Class
{
	...
	{
		self::$Conexion = new mysqli(self::$servidor, self::$usuario, self::$clave, self::$db);

		if (self::$Conexion->connect_error) {
			header('location:/');
		} else {
			self::$Conexion->query("SET NAMES " . self::$encoding);
		}
	}
	...
	{
		self::$Conexion = self::$Conexion->free_result(self::$Query_ID);
	...
	public static function DB_Consulta($sql = "")
	{
		...
		try {
			...
			self::$Query_ID = self::$Conexion->query($sql);
	...
	public static function DB_Insertar($tabla, $into, $values)
	{
		...
		try {
			...
			self::$Query_ID = @self::$Conexion->query($query);
